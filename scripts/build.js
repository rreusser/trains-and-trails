...
copy(
  join("..", "node_modules", "mapbox-gl", "dist", "mapbox-gl.js"),
  "mapbox-gl.js"
);
copy(
  join("..", "node_modules", "mapbox-gl", "dist", "mapbox-gl.css"),
  "mapbox-gl.css"
);
copy(join("..", "node_modules", "d3", "dist", "d3.min.js"), "d3.min.js");
copy(join("..", "src", "styles.css"), "styles.css");
copy(join("..", "src", "pin.png"), "pin.png");
...
async function processPage(mdPath) {
  ...
  const { html, page } = await buildPage(
    mdContent,
    url,
    siteManifest,
    thisroute,
