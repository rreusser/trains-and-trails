(function (global, factory) {
  ...
  (global = typeof globalThis !== 'undefined' ? globalThis : global || self, global.MapboxThirdPersonCamera = factory());
  ...
  var CameraController = /*#__PURE__*/function () {
    function CameraController(map) {
      ...
      this.map = map;
      this.MercatorCoordinate = this.map.getFreeCameraOptions().position.constructor;
      this._distanceTarget = distance;
      this._pitchTarget = pitch;
      this._bearingTarget = bearing;
      this._centerTarget = this.center = this.MercatorCoordinate.fromLngLat(map.transform.center);
    ...
    _createClass(CameraController, [{
      ...
      set: function set(center) {
        ...
        this._centerTarget = this.MercatorCoordinate.fromLngLat(center, elevation);
      }
    }, {
      key: "clear",
      ...
      value: function setPanPID() {
        var P = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : 2.5;
        var I = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 1.5;
        var D = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : 150;
        this.Kp_pan = P;
        this.Ki_pan = I;
        this.Kd_pan = D;
      }
      ...
      value: function tick(t) {
        ...
        var toMeters = 1 / this._centerTarget.meterInMercatorCoordinateUnits();
        ...
        if (this._center === null) {
          ...
          var elevation = this.map.queryTerrainElevation(this._centerTarget.toLngLat()) || 0;
          ...
          cam.position.x = this._center.x + Math.sin(phi) * Math.sin(theta) * dist;
          cam.position.y = this._center.y + Math.sin(phi) * Math.cos(theta) * dist;
          cam.position.z = this._center.z + Math.cos(phi) * dist + elevation / toMeters;
        ...
        var dt = isNaN(this._previousTime) ? 0 : Math.min(t - this._previousTime, 32) / 1000;
        this._previousTime = t; // Compensate for terrain being added/removed

        if (hasTerrain && !this._hadTerrain) {
          var el = this.map.queryTerrainElevation(this._centerTarget.toLngLat());
        ...
        var mPanErr = [this._centerTarget.x - this._center.x, this._centerTarget.y - this._center.y, this._centerTarget.z - this._center.z]; // Use only the first two coordinates!
        ...
        var sky = [0, 0, 1];
        var vcam = [this._centerTarget.x - cam.position.x, this._centerTarget.y - cam.position.y, this._centerTarget.z - cam.position.z];
        ...
        var bearingTarget = [Math.cos(this._bearingTarget * (Math.PI / 180)), Math.sin(this._bearingTarget * (Math.PI / 180))];
        ...
        if (targetErrorMeters * panp > this._distanceTarget / 1000 || Math.abs(camDistForce) * distp > 1e-8 || Math.abs(pitchForce) * pitchp > 1e-8 || length_1(bearingForce) * bearingp > 1e-8) {
          var dlookdt = [0, 0, 0];

          if (this._previousCenter && !isNaN(this._previousLookAtTime) && t > this._previousLookAtTime) {
          ...
          var panx = (mPanErr[0] * panp + this._errorIntegral[0] * pani + dlookdt[0] * pand) * dt;
          var pany = (mPanErr[1] * panp + this._errorIntegral[1] * pani + dlookdt[1] * pand) * dt;
          var panz = (mPanErr[2] * panp + this._errorIntegral[2] * pani + dlookdt[2] * pand) * dt; // Mutate the look-at point in-place
